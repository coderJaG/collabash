...
const PotDetailsPage = () => {
    ...
    // Add selectors for pot loading/error state from the pots slice if available
    // const isLoadingPot = useSelector(state => state.pots.isLoading);
    // const potError = useSelector(state => state.pots.error);
    ...
    useEffect(() => {
        ...
        if (numPotId && !isLoadingPot && potDetails) {
            
            dispatch(fetchWeeklyStatus(numPotId, currentWeek));
        }
    }, [dispatch, numPotId, currentWeek, isLoadingPot, potDetails]);
